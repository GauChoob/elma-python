...
class TestLGR(unittest.TestCase):
    ...
    def test_packing_and_unpacking(self):
        ...
        self.assertEqual(lgr1.version, lgr2.version)
        self.assertEqual(lgr1.palette, lgr2.palette)
        for k in range(len(lgr1.images)):
            self.assertEqual(lgr1.images[k], lgr2.images[k])
        # LGR13
        lgr1 = unpack_LGR('tests/files/steam.lgr')
        with open('tests/files/result/steam.lgr', 'wb') as f:
            f.write(pack_LGR(lgr1))
        # not comparing binaries here because the script changes pcx header &
        # compresses a bit better than original .pcx files
        lgr2 = unpack_LGR('tests/files/result/steam.lgr')
        self.assertEqual(lgr1.version, lgr2.version)
        self.assertEqual(lgr1.palette, lgr2.palette)
        for k in range(len(lgr1.images)):
            self.assertEqual(lgr1.images[k], lgr2.images[k])
        # LGR12 turning into LGR13
        lgr1.version = 13
        pack_LGR(lgr1)
        lgr1.version = 12
        # LGR12 trying to use LGR13 properties
        backup = lgr1.images[0].height
        lgr1.images[0].height = 3
        self.assertRaises(ValueError, lambda: pack_LGR(lgr1))
        lgr1.images[0].height = backup
        lgr1.images[0].width = 3
        self.assertRaises(ValueError, lambda: pack_LGR(lgr1))
...
class TestLGRErrors(unittest.TestCase):
    ...
    def setUp(self):
        self.lgr12 = unpack_LGR('tests/files/default.lgr')
        self.lgr13 = unpack_LGR('tests/files/steam.lgr')
        os.makedirs('tests/files/result', exist_ok=True)
    ...
    def test_error_no_errors(self):
        self.assertEqual(check_LGR_error(self.lgr12), [])
        self.assertEqual(check_LGR_error(self.lgr13), [])

    def test_error_invalid_palette(self):
        self.lgr12.palette[0] = 300
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
                         elma.error.ERR_LGR_INVALID_PALETTE)
        self.lgr12.palette[0] = -10
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
                         elma.error.ERR_LGR_INVALID_PALETTE)
        self.lgr12.palette[0] = 0
        self.lgr12.palette.append([0])
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
                         elma.error.ERR_LGR_INVALID_PALETTE)

    def test_error_missing_mandatory_file(self):
        self.lgr12.images.pop(0)
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
                         elma.error.ERR_LGR_MISSING_MANDATORY_FILE)

    def test_error_duplicate_name(self):
        self.lgr12.images.append(self.lgr12.images[-1])
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
    ...
    def test_error_too_many_grass(self):
        for i in range(30, 80):  # skip the already-present qups and qdowns
            ...
            imgt.putpalette(self.lgr12.palette)
            self.lgr12.images.append(LGR_Image(
                'qUp_%s' % i,
                imgt,
                LGR_Image.PICTURE,
                999,
    ...
    def test_error_not_enough_textures(self):
        del self.lgr12.images[self.lgr12.find_LGR_Image("brick")]
        del self.lgr12.images[self.lgr12.find_LGR_Image("ground")]
        del self.lgr12.images[self.lgr12.find_LGR_Image("sky")]
        del self.lgr12.images[self.lgr12.find_LGR_Image("stone1")]
        del self.lgr12.images[self.lgr12.find_LGR_Image("stone2")]
        del self.lgr12.images[self.lgr12.find_LGR_Image("stone3")]
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
    ...
    def test_error_invalid_version(self):
        self.lgr12.version = 11
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
                         elma.error.ERR_LGR_INVALID_VERSION)
        self.lgr12.version = 14
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
                         elma.error.ERR_LGR_INVALID_VERSION)
        self.lgr12.version = "13"
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
                         elma.error.ERR_LGR_INVALID_VERSION)

    def test_error_scaled_image_in_lgr12(self):
        self.lgr12.images[-1].width = 50000
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
                         elma.error.ERR_SCALED_IMAGE_IN_LGR12)
        self.lgr12.images[-1].width = -1
        self.lgr12.images[-1].height = 50000
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
                         elma.error.ERR_SCALED_IMAGE_IN_LGR12)
    ...
    def test_warning_unused_qfood(self):
        ...
        imgt.putpalette(self.lgr12.palette)
        self.lgr12.images.append(LGR_Image('qfOod4', imgt))
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
                         elma.error.WARN_UNUSED_QFOOD)

    def test_error_name_too_long(self):
        self.lgr12.images[-1].name = 'nameTOOlong'
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
                         elma.error.ERR_NAME_TOO_LONG)

    def test_error_name_missing(self):
        self.lgr12.images[-1].name = ''
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
                         elma.error.ERR_NAME_MISSING)

    def test_error_padding_invalid(self):
        self.lgr12.images[-1].name = "x"
        self.lgr12.images[-1].padding = [1, 2, 3, 4, 5, 6, 7, 8]
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
                         elma.error.ERR_PADDING_INVALID)
        self.lgr12.images[-1].padding = [1, 2, 3, 4, 5, 600, 7]
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
                         elma.error.ERR_PADDING_INVALID)
        self.lgr12.images[-1].padding = [1, 2, 3, 4, 5, 'error', 7]
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
                         elma.error.ERR_PADDING_INVALID)
    ...
    def test_warning_grass_height_too_small(self):
        ...
        imgt.putpalette(self.lgr12.palette)
        self.lgr12.images[-1].name = "qUp_Xx"
        self.lgr12.images[-1].img = imgt
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
                         elma.error.WARN_GRASS_HEIGHT_TOO_SMALL)

    def test_error_obj_width_invalid_12(self):
        imgt = Image.new('P', (41, 40), 0)
        imgt.putpalette(self.lgr12.palette)
        qexit_index = self.lgr12.find_LGR_Image("qexit")
        original_qexit_image = self.lgr12.images[qexit_index].img
        self.lgr12.images[qexit_index].img = imgt
        self.lgr12.images[qexit_index].name = "qeXit"
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
                         elma.error.ERR_OBJ_WIDTH_INVALID)
        self.lgr12.images[qexit_index].img = original_qexit_image
        self.lgr12.images[-1].name = "qFOOd3"
        self.lgr12.images[-1].img = imgt
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
                         elma.error.ERR_OBJ_WIDTH_INVALID)

    def test_error_obj_width_invalid_13(self):
        imgt = Image.new('P', (40, 60), 0)
        imgt.putpalette(self.lgr13.palette)
        self.lgr13.images[-1].name = "qFOOd3"
        self.lgr13.images[-1].img = imgt
        self.lgr13.images[-1].width = 27
        self.lgr13.images[-1].height = 40
        self.assertEqual(check_LGR_error(self.lgr13)[0][0],
                         elma.error.ERR_OBJ_WIDTH_INVALID)

    def test_error_obj_height_invalid_12(self):
    ...
    def test_error_obj_height_invalid_13(self):
        imgt = Image.new('P', (80, 2), 0)
        imgt.putpalette(self.lgr13.palette)
        self.lgr13.images[-1].name = "qFOOd3"
        self.lgr13.images[-1].img = imgt
        self.lgr13.images[-1].width = 40
        self.lgr13.images[-1].height = 1
        self.assertEqual(check_LGR_error(self.lgr13)[0][0],
                         elma.error.WARN_OBJ_HEIGHT_INVALID)
    ...
    def test_error_image_type_invalid(self):
        ...
        imgt.putpalette(self.lgr12.palette)
        self.lgr12.images[-1].img = imgt
        self.lgr12.images[-1].image_type = 30
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
    ...
    def test_error_distance_invalid(self):
        ...
        imgt.putpalette(self.lgr12.palette)
        self.lgr12.images[-1].img = imgt
        self.lgr12.images[-1].default_distance = 0
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
                         elma.error.ERR_DISTANCE_INVALID)
        self.lgr12.images[-1].default_distance = 1000
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
                         elma.error.ERR_DISTANCE_INVALID)
        self.lgr12.images[-1].image_type = LGR_Image.MASK
        self.assertEqual(check_LGR_error(self.lgr12), [])

    def test_error_clipping_invalid(self):
        self.lgr12.images[-1].image_type = LGR_Image.MASK
        self.lgr12.images[-1].default_clipping = 10
        self.assertEqual(check_LGR_error(self.lgr12), [])
        self.lgr12.images[-1].image_type = LGR_Image.TEXTURE
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
                         elma.error.ERR_CLIPPING_INVALID)
        self.lgr12.images[-1].default_clipping = -1
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
                         elma.error.ERR_CLIPPING_INVALID)

    def test_error_transparency_invalid(self):
        self.lgr12.images[-1].transparency = 100
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
                         elma.error.ERR_TRANSPARENCY_INVALID)
        self.lgr12.images[-1].transparency = -1
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
                         elma.error.ERR_TRANSPARENCY_INVALID)
    ...
    def test_warning_palette_mismatch(self):
        ...
        self.lgr12.images[4].put_palette(edited_palette)
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
    ...
    def test_warning_qcolors_wrong_size(self):
        index = self.lgr12.find_LGR_Image('qCOlorS')
        imgt = Image.new('P', (67, 109), 0)
        imgt.putpalette(self.lgr12.palette)
        self.lgr12.images[index].img = imgt
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
    ...
    def test_warning_qbike_too_small(self):
        index = self.lgr12.find_LGR_Image("q2BIKE")
    ...
    def test_warning_cannot_scale(self):
        index = self.lgr13.find_LGR_Image("q2BIKE")
        backup_width = self.lgr13.images[index].width
        self.lgr13.images[index].width = 600
        self.assertEqual(check_LGR_error(self.lgr13)[0][0],
                         elma.error.WARN_IGNORED_SCALING)
        self.lgr13.images[index].width = backup_width
        self.lgr13.images[index].height = backup_width
        self.lgr13.images[index].height = 600
        self.assertEqual(check_LGR_error(self.lgr13)[0][0],
                         elma.error.WARN_IGNORED_SCALING)

    def test_error_img_missing(self):
        self.lgr12.images[0].img = None
        self.assertEqual(check_LGR_error(self.lgr12)[0][0],
